...
#include <actionlib_tutorials/random_number.h>
...
public:
  ...
  void analysisCB(const actionlib_tutorials::random_number& msg)
  {
    ...
    feedback_.data = msg.random_number;
    //compute the std_dev and mean of the data 
    sum_ += msg.random_number;
    feedback_.mean = sum_ / data_count_;
    sum_sq_ += pow(msg.random_number, 2);
    feedback_.std_dev = sqrt(fabs((sum_sq_/data_count_) - pow(feedback_.mean, 2)));
    as_.publishFeedback(feedback_);
    ROS_INFO("feedback_data:%f, feedback_sum:%f, feedback_.mean:%f, feedback_.std_dev:%f",feedback_.data,sum_,feedback_.mean,feedback_.std_dev);
